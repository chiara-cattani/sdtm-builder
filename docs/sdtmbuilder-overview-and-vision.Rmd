...
- Each variable's `DERIVATION` column contains a derivation instruction:
...
- **Programmer effort**: fill in Variables sheet with DERIVATION expressions.
  Zero R code.
...
| **Rules** (C) | `compile_rules()`, `enrich_rules_with_ct()` | Parse DERIVATION column into executable rules |
...
| **Derivations** (G1–G6) | `map_direct()`, `derive_constant()`, `assign_ct()`, `parse_partial_date()`, `format_iso_dtc()`, `derive_dy()`, `derive_seq()`, `derive_usubjid()`, `derive_dict_version()`, `derive_case_when()`, `derive_if_else()`, `derive_coalesce()` | Individual variable derivations |
...
phases <- data.frame(
  ...
  What_the_Programmer_Provides = c(
    ...
    "Variables sheet (DERIVATION column)",
...
DERIVATION expressions like `direct(aeterm)`, `format_date(aestdat)`,
