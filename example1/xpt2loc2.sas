...
* PROGRAM NAME  : xpt2loc2.sas
* PURPOSE       : Convert a SAS transport file (.xpt) into one or more datasets in a local library.
*                 Supports both V5 and V8 formats with automatic metadata reconstruction.
 ------------------------------------------------------------------------------
* NOTES         :
* - Used internally by SDTM macros such as %compare_qc.
* - Requires companion macros: %xptcommn, %xprtflt, %make_memwant_fmt, %make_nliteral.
* - For V8 files, ensure dataset names match expected domain names to avoid skipping.
* DESCRIPTION   :
* - Parses the transport file and generates SAS DATA step code to recreate datasets.
* - Handles descriptor records, variable attributes, formats, and labels.
* - Creates macro variables for observation offsets and lengths (e.g., &firstobs_mem0001).
* - Supports RECFM=N streaming input for non-MVS systems; includes fallback logic for MVS.
* PARAMETERS    :
* - libref=     : Target library for output datasets (default: WORK)
* - memlist=    : List of dataset members to convert (default: _ALL_)
* - filespec=   : Fileref or quoted path to the transport file (required)
 ------------------------------------------------------------------------------
* PROGRAM HISTORY :
* 2025-10-10 - cattanch - Initial version
***********************************************************************************************/

%*-------------------------------------------------------------------*;
%* The xpt2loc2 macro is used to convert a transport file into local  *;
...
%macro xpt2loc2(libref=work,memlist=_all_,filespec=);
...
filename inptstmt temp recfm=v lrecl=32767;
filename metadata temp recfm=v lrecl=32767;
