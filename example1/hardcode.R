...
#'
#' @description
#' [sdtm_hardcode_cc()] is an internal function packing the same functionality as
...
#' @importFrom rlang :=
#' @keywords internal
sdtm_hardcode_cc <- function(tgt_dat = NULL,
                          tgt_var,
                          raw_dat,
                          raw_var,
                          tgt_val,
  ...
  # Ensure cur_tgt_val is a character vector of correct length
  cur_tgt_val <- if (!tgt_var %in% names(join_dat)) {
    rep(NA_character_, nrow(join_dat))
  } else {
    as.character(join_dat[[tgt_var]])
  }
  
  # Ensure raw_var is character before recoding
  join_dat[[raw_var]] <- as.character(join_dat[[raw_var]])
  
  # Apply recoding safely
  join_dat <- join_dat |>
    
    dplyr::mutate(
      "{tgt_var}" := dplyr::coalesce(cur_tgt_val, tgt_val)
    ) |>
