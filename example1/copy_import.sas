...
PURPOSE     :   Copy a CRF data set to work, create USUBJID, SUBJID and remove formats and informat
--------------------------------------------------------------------------------
NOTES:          Copied and adapted from YoFlow.
--------------------------------------------------------------------------------
PROGRAM HISTORY:
2025-04-14  Chiara Cattani - Initial program
********************************************************************************/
%macro copy_import (inset =                   /* input datset */        
   ...
   proc sql;
      ...
      select catx('-', "&studyid", scan(subjectid, -3, '-'), scan(subjectid, -2, '-'), put(input(scan(subjectid,-1, '-'), best.), z4.)) as USUBJID length=50
      , *
      from &inlib..&inset. 
