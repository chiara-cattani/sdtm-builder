...
for (subj in subjects) {
  ...
  for (j in med_idx) {
    ...
    indic_val  <- indications[((j - 1) %% length(indications)) + 1]
    ...
    rows <- c(rows, list(tibble(
      ...
      cmatc3          = med$cmatc3,
      cmatc4          = med$cmatc4,
      DictInstance    = "WHODrug B3 September 2023",
...
# =============================================================================
# cmae1 : AESPID of a related Adverse Event (e.g., medication given for an AE)
# cmmh1 : MHSPID of a related Medical History condition (indication = MH term)
#
# These link columns mirror the EDC structure seen in real studies where the
# CRF allows the investigator to link a concomitant medication record to an
# AE and/or MH that prompted the medication.
# -----------------------------------------------------------------------------

# Read AE and MH to create plausible links
ae  <- read.csv("raw/ae.csv",  stringsAsFactors = FALSE)
mh  <- read.csv("raw/mh.csv",  stringsAsFactors = FALSE)

cm_whodrug$cmae1 <- NA_character_
cm_whodrug$cmmh1 <- NA_character_

# Link ~30% of CMs to an AE from the same subject
set.seed(1234)
for (i in seq_len(nrow(cm_whodrug))) {
  subj <- cm_whodrug$subjectid[i]
  subj_aes <- ae[ae$SubjectId == subj, ]
  subj_mhs <- mh[mh$SubjectId == subj, ]

  # Link to AE with 35% probability

  if (nrow(subj_aes) > 0 && runif(1) < 0.35) {
    cm_whodrug$cmae1[i] <- as.character(sample(subj_aes$AESPID, 1))
  }

  # Link to MH with 40% probability (medications often treat MH conditions)
  if (nrow(subj_mhs) > 0 && runif(1) < 0.40) {
    cm_whodrug$cmmh1[i] <- as.character(sample(subj_mhs$MHSPID, 1))
  }
}
