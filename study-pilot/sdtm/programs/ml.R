# ******************************************************************************
# STUDY-PILOT
# ******************************************************************************
# PROGRAM NAME  : ML.R
# PURPOSE       : SDTM ML Domain - Meal Data
# ------------------------------------------------------------------------------
# NOTES :
#   Raw datasets : ml
#   Dependencies : sdtmbuilder package
# ------------------------------------------------------------------------------
# PROGRAM HISTORY :
# 2026-02-15 - sdtmbuilder - Auto-generated program
# ******************************************************************************

# Configuration ----
library(sdtmbuilder)
library(dplyr)

study       <- "STUDY-PILOT"
sdtm_domain <- "ML"

# Set working directory to study root ----
if (requireNamespace("rstudioapi", quietly = TRUE) &&
    rstudioapi::isAvailable()) {
  prog_dir <- dirname(rstudioapi::getActiveDocumentContext()$path)
  setwd(file.path(prog_dir, "..", ".."))
} else if (exists("progdir")) {
  setwd(file.path(progdir, "..", ".."))
}

# Import metadata ----
study_meta <- read_study_metadata_excel("metadata/Study_Metadata.xlsx")
ct_spec    <- read_study_ct_excel("metadata/Study_CT.xlsx")

target_meta      <- study_meta$target_meta
domain_meta      <- study_meta$domain_meta
value_level_meta <- study_meta$value_level_meta

# Import data ----
all_raw <- load_raw_datasets("raw")
for (nm in names(all_raw)) {
  all_raw[[nm]] <- all_raw[[nm]] %>% standardize_names() %>% convert_blanks_to_na()
}

ml <- all_raw[["ml"]]
dm1 <- all_raw[["dm"]]

# Sources lookup (for SOURCEID) ----
review_status <- all_raw[["review_status"]]
sources <- review_status %>%
  dplyr::filter(trimws(formname) != "") %>%
  dplyr::distinct(formid, formname)

# Prepare DM for RFSTDTC ----
# Load DM SDTM dataset (has RFSTDTC); fall back to raw DM
dm_rda <- "sdtm/datasets/RDA/dm.rda"
dm_xpt <- "sdtm/datasets/XPT/dm.xpt"
if (file.exists(dm_rda)) {
  dm_env <- new.env(parent = emptyenv())
  load(dm_rda, envir = dm_env)
  dm_sdtm <- dm_env[[ls(dm_env)[1]]]
  names(dm_sdtm) <- tolower(names(dm_sdtm))
} else if (file.exists(dm_xpt)) {
  dm_sdtm <- haven::read_xpt(dm_xpt)
  names(dm_sdtm) <- tolower(names(dm_sdtm))
} else {
  dm_sdtm <- dm1
}

dm_slim <- dm_sdtm %>%
  dplyr::select(dplyr::any_of(c("subjectid", "subjid", "usubjid", "rfstdtc"))) %>%
  dplyr::distinct()

# Join RFSTDTC: try subjectid=subjid, fall back to usubjid
if ("subjectid" %in% names(ml) && "subjid" %in% names(dm_slim)) {
  ml <- ml %>%
    dplyr::left_join(dm_slim %>% dplyr::select(dplyr::any_of(c("subjid", "rfstdtc"))) %>% dplyr::rename(subjectid = subjid), by = "subjectid")
} else if ("usubjid" %in% names(ml) && "usubjid" %in% names(dm_slim)) {
  ml <- ml %>%
    dplyr::left_join(dm_slim %>% dplyr::select(dplyr::any_of(c("usubjid", "rfstdtc"))), by = "usubjid")
}
if ("rfstdtc" %in% names(ml) && !"RFSTDTC" %in% names(ml)) ml$RFSTDTC <- ml$rfstdtc

# ML derivations ----
ml2 <- ml %>%
  # --- Identifiers ---
  mutate(
    STUDYID = "STUDY-PILOT",
    DOMAIN = "ML",
    MLSPID = as.character(mlspid_derived)
  ) %>%
  # --- Derived Variables ---
  mutate(MLLNKID = as.character(mllnkid_derived)) %>%
  # --- Topic / Term ---
  mutate(MLTRT = as.character(mltrt_derived)) %>%
  # --- Categories ---
  mutate(
    MLCAT = as.character(mlcat_derived),
    MLSCAT = as.character(mlscat_derived)
  ) %>%
  # --- Derived Variables ---
  mutate(
    MLPRESP = as.character(mlpresp_derived),
    MLOCCUR = as.character(mloccur_derived),
    MLDOSE = as.numeric(mldose_derived),
    MLDOSTXT = as.character(mldostxt_derived),
    MLDOSU = as.character(mldosu_derived),
    MLDOSFRQ = as.character(mldosfrq_derived)
  ) %>%
  # --- Visit Variables ---
  mutate(
    VISITNUM = as.numeric(visitnum_derived),
    VISIT = as.character(visit_derived),
    VISITDY = as.numeric(visitdy_derived)
  ) %>%
  # --- Derived Variables ---
  mutate(
    MLSTDTC = as.character(mlstdtc_derived),
    MLENDTC = as.character(mlendtc_derived),
    MLEVINTX = as.character(mlevintx_derived)
  ) %>%
  # --- Coded Fields ---
  mutate(
    MLENRTPT = as.character(mlenrtpt_derived),
    MLENTPT = as.character(mlentpt_derived)
  ) %>%
  # --- Identifiers ---
  mutate(
    SOURCEID = paste("CRF:", sources$formname[match("ML", toupper(sources$formid))]),
    SUBJID = as.character(subjectid)
  ) %>%
  derive_usubjid(study, subjid_col = "subjectid") %>%
  # --- Study Day ---
  derive_dy("MLSTDY", "MLSTDTC", "RFSTDTC") %>%
  derive_dy("MLENDY", "MLENDTC", "RFSTDTC") %>%
  # --- Sequence ---
  derive_seq("MLSEQ", by = c("USUBJID"))

# Finalize ----
ml_final <- export_domain(
  data        = ml2,
  domain      = sdtm_domain,
  output_dir  = "sdtm/datasets",
  formats     = c("xpt", "rda"),
  xpt_version = 8L,
  target_meta = target_meta,
  domain_meta = domain_meta,
  keys        = c("STUDYID", "USUBJID", "MLTRT", "MLSTDTC", "MLSPID"),
  drop_empty_perm = TRUE
)

cli::cli_alert_success("ML domain created: {nrow(ml2)} rows")
