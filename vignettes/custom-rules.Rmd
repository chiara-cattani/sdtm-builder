...
# ── Compile rules (auto-assigns rule_type for DERIVATION=NA) ─
rule_set <- compile_rules(target_meta, ct_lib = ct_lib)
```

`compile_rules()` examines the `DERIVATION` column in `target_meta`:

| DERIVATION value | Resulting `rule_type` | Parameters |
|---|---|---|
...
`Study_Metadata.xlsx`. Set the `DERIVATION` column to a function-call string:

| DOMAIN | VARIABLE | DERIVATION |
|--------|----------|--------|
| AE | AESEV | `{"rule_type": "assign_ct", "params": {"codelist": "SEV", "dataset": "ae_raw", "column": "severity"}}` |

When `compile_rules()` encounters a JSON string in DERIVATION, it parses it
...
- **Use function-call syntax in DERIVATION**: for rules that can be expressed declaratively,
  the DERIVATION column in the Excel metadata is the most maintainable approach.
