...
ae_meta[, c("var", "type", "core", "rule_type")]
```
...
- **`rule_type`** — the derivation method (`constant`, `direct_map`, `ct_assign`,
  `iso_dtc`, `dy`, `seq`)
- **`DERIVATION`** — the derivation expression (e.g. `derive_dy(dtc_var = "AESTDTC")`)

Derivation order is computed automatically from variable names and rule types
using the mandatory 10-category SDTM sequence (no manual dependency tracking).
...
1. Parses the DERIVATION column syntax (e.g. `derive_dy(dtc_var = "AESTDTC")`)
2. Auto-assigns rule\_type for variables without explicit DERIVATION
3. Infers derivation order automatically from the mandatory SDTM sequence
