...
| DERIVATION | character | No | Derivation rule — NA for auto-assign, or function-call string |
| VALUE | character | No | Constant value (when DERIVATION implies constant) |
| ORDER | integer | No | Display order within the domain |

#### DERIVATION Auto-Assignment

When `DERIVATION` is `NA`, `compile_rules()` applies these defaults:
...
| DERIVATION | character | No | Override derivation method |
