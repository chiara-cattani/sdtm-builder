...
| `seq:USUBJID, AESPID` | `derive_seq(by = c("USUBJID", "AESPID"))` | Sequence grouped by multiple variables |
...
Derive sequence by one or more grouping variables.

```
seq:USUBJID             →  derive_seq(by = "USUBJID")
seq:USUBJID, AESPID     →  derive_seq(by = c("USUBJID", "AESPID"))
```
