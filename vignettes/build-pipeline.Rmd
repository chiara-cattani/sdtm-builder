...
│                             topo_sort_rules()                   │
│  Step 6 ─ Load Data   ───► load_raw_data() / load_raw_datasets()│
...
file needed. Use `infer_source_meta(raw_data)` if you need it explicitly.
...
for (dom in names(results)) {
  export_xpt(results[[dom]]$data, dom, output_dir, mb$target_meta)
  export_rds_csv(results[[dom]]$data, dom, output_dir)
}
```
...
export_xpt(ae$data, "AE", tempdir(), study$target_meta)
```
